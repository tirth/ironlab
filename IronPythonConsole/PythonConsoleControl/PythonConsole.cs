...
namespace PythonConsoleControl
{
    ...
    public class PythonConsole : IConsole, IDisposable
    {
        ...
        readonly int _lineReceivedEventIndex = 0; // The index into the waitHandles array where the lineReceivedEvent is stored.
        ...
        AutoResetEvent _statementsExecutionRequestedEvent = new AutoResetEvent(false);
        ...
        {   
            ...
            _textEditor.TextArea.Dispatcher.Invoke(new Action(delegate
            {
                ...
                this._textEditor.TextArea.CommandBindings.Add(new CommandBinding(ApplicationCommands.Paste, OnPaste, CanPaste));
                this._textEditor.TextArea.CommandBindings.Add(new CommandBinding(ApplicationCommands.Copy, OnCopy, PythonEditingCommandHandler.CanCutOrCopy));
                this._textEditor.TextArea.CommandBindings.Add(new CommandBinding(ApplicationCommands.Cut, PythonEditingCommandHandler.OnCut, CanCut));
                this._textEditor.TextArea.CommandBindings.Add(new CommandBinding(ApplicationCommands.Undo, OnUndo, CanUndo));
                this._textEditor.TextArea.CommandBindings.Add(new CommandBinding(ApplicationCommands.Delete, PythonEditingCommandHandler.OnDelete(ApplicationCommands.NotACommand), CanDeleteCommand));
        ...
        protected void OnPaste(object target, ExecutedRoutedEventArgs args)
        {
            ...
            if (textArea != null && textArea.Document != null)
            {
                ...
                if (commands.Length > 1)
                {
                    ...
                    _dispatcherWindow.Dispatcher.BeginInvoke(new Action(delegate { ExecuteStatements(); }));
                }
            }
        }
        ...
        public void RunStatements(string statements)
        {
            ...
            _dispatcher.BeginInvoke(new Action(delegate { ExecuteStatements(); }));
        }
        ...
        {
            var fullLine = GetLastTextEditorLine();
        ...
        {
            var result = WaitHandle.WaitAny(_waitHandles);
            if (result == _lineReceivedEventIndex)
            {
                lock (_previousLines)
                {
                    var line = _previousLines[0];
        ...
        /// </summary>
        bool IsInReadOnlyRegion => IsCurrentLineReadOnly || IsInPrompt;

        /// <summary>
        /// Only the last line in the text editor is not read only.
        /// </summary>
        bool IsCurrentLineReadOnly => _textEditor.Line < _textEditor.TotalLines;

        /// <summary>
        /// Determines whether the current cursor position is in a prompt.
        /// </summary>
        bool IsInPrompt => _textEditor.Column - _promptLength - 1 < 0;
        ...
        bool SelectionIsDeletable => (!_textEditor.SelectionIsMultiline
